...
f = @(x, y, z) (x - 0).^2 + (y + 0).^2 + (z + 1).^2 - 0.4.^2;
...
% initializing sphere with the center in (0, 0, -1) and with radius 0.5
f1 = @(x, y, z) (x - 0).^2 + (y - 0.5).^2 + (z + 0.5).^2 - 0.25.^2;
% computing partial derivatives to the parametrization of the sphere
df1dx = @(x, y, z) 2.*x;
df1dy = @(x, y, z) 2.*(y - 0.5);
df1dz = @(x, y, z) 2.*(z + 0.5);
...
for i=1:image_height
    ...
    def_colors = [1 0 0; 0 1 0]';
    image(i, j, :) = raytracing(def_colors, f, f1, dfdx, dfdy, dfdz, df1dx, df1dy, df1dz, origin, direction_vector, lightOrigin, 0.01, 200, 10);
    end
end
